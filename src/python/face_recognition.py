...
class TrackerState:
    def __init__(self, tracker_id):
        ...
        self.tracking_active = self.tracker is not None
        ...
        self.bbox = None  # Store current bbox for fallback
        
        if not self.tracking_active:
            logger.warning(f"Tracker {tracker_id} created without OpenCV tracker - using detection only")
    ...
    def update_tracker(self, frame):
        ...
        if self.tracker is not None and self.tracking_active:
            try:
            ...
            # Return stored bbox if no tracker
            return self.bbox is not None, self.bbox
...
if __name__ == "__main__":
    ...
    # Test tracker compatibility first
    test_tracker = create_tracker()
    if test_tracker is None:
        logger.warning("⚠️ No OpenCV tracker available. Using detection-only mode.")
        print("⚠️ No OpenCV tracker available. Using detection-only mode.")
        use_tracking = False
    else:
        logger.info("✅ OpenCV tracker test successful")
        print("✅ OpenCV tracker available")
        use_tracking = True
    ...
    detection_interval = FaceRecognitionConfig.DETECTION_INTERVAL # Số frame giữa các lần detect mới
    recognition_interval = 3  # Chỉ inference 3 frame 1 lần
    frame_count = 0
    ...
    logger.info(f"🔧 Tracking mode: {'Enabled' if use_tracking else 'Detection-only'}")
    ...
    while cap.isOpened():
        ...
        need_detection = (len(tracker_states) == 0 or 
                         ...
                         (use_tracking and not any(ts.tracking_active for ts in tracker_states)))
        
        if need_detection:
            # Sử dụng MTCNN để phát hiện các khuôn mặt trong khung hình
            ...
            if boxes is not None:
                ...
                # Tạo danh sách trạng thái tracker mới
                new_tracker_states = []
                
                # Duyệt qua từng khuôn mặt được phát hiện
                for i, box in enumerate(boxes):
                    # Tạo một trạng thái tracker mới với ID tương ứng
                    tracker_state = TrackerState(tracker_id=len(new_tracker_states))
                    
                    # Chuyển đổi bounding box từ format [x1, y1, x2, y2] sang [x, y, width, height]
                    bbox = (int(box[0]), int(box[1]), int(box[2]-box[0]), int(box[3]-box[1]))
                    
                    # Khởi tạo tracker với khung hình và bounding box hiện tại
                    if use_tracking:
                        success = tracker_state.init_tracker(frame, bbox)
                        if success or not tracker_state.tracking_active:
                            new_tracker_states.append(tracker_state)
                            logger.debug(f"   Tracker {i}: bbox={bbox}, tracking={tracker_state.tracking_active}")
                    else:
                        # Detection-only mode
                        tracker_state.bbox = bbox
                        new_tracker_states.append(tracker_state)
                        logger.debug(f"   Detection {i}: bbox={bbox}")
                
                # Cập nhật danh sách tracker states
        ...
        for tracker_state in tracker_states:
            # Cập nhật vị trí tracker trong frame hiện tại
            if use_tracking:
                success, bbox = tracker_state.update_tracker(frame)
            else:
                # Detection-only mode
                success = tracker_state.bbox is not None
                bbox = tracker_state.bbox
            
            # Nếu tracker còn theo dõi được
            if success and bbox is not None:
                # Thêm vào danh sách tracker đang hoạt động
                active_trackers.append(tracker_state)
                
                # Chuyển đổi bbox sang các giá trị số nguyên
                ...
                # Quyết định có nên chạy nhận diện hay không dựa trên interval
                should_infer = tracker_state.should_run_inference(frame_count, recognition_interval)
                
                # Nếu đến lượt chạy nhận diện
                if should_infer:
                    ...
                    if face_roi.size > 0:
                        ...
                        # Cố gắng trích xuất tensor khuôn mặt bằng MTCNN
                        try:
                            ...
                            logger.debug(f"MTCNN unexpected error: {e}")
                            face_tensor = None
                        
                        # Nếu có tensor khuôn mặt
                        if face_tensor is not None:
                            # Cập nhật các số liệu thống kê
                            total_faces_processed += 1
                            total_inferences_run += 1
                            ...
                            # Đánh dấu việc nhận diện thất bại
