...
public:
  ...
  // Preprocess a pre-cropped face ROI into normalized tensor
  // This is for cases where you already have a bounding box from detection
  // and just want to normalize/crop without re-detecting
  std::vector<float> preprocessFace(const cv::Mat &faceRoi, const int imageSize = 160,
                                    const int margin = 0);
};
