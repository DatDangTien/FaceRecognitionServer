...
class FaceRecognitionWebSocketServer:
    def __init__(self, db_config=None):
        ...
        self.db = FaceDatabase(connection_params=db_config)
        
        # Load embeddings and usernames from database
    ...
    def load_faceslist(self):
        """Load existing embeddings and usernames from database"""
        try:
            # Try loading from database
            embeddings, names = self.db.get_all_users()
            ...
            if embeddings is not None and names is not None:
                ...
                logger.info(f"✅ Loaded embeddings from database: {embeddings.shape}")
                logger.info(f"✅ Loaded usernames from database: {names}")
                return embeddings, names
            
            # If database is empty, try loading from files as fallback
            logger.warning("Database is empty. Trying to load from files as fallback...")
            return self.load_faceslist_from_files()
    ...
    def load_faceslist_from_files(self):
        """Legacy method to load embeddings and usernames from files"""
        import os
        ...
        if os.path.exists(embedding_file):
            ...
            # Ensure embeddings are on correct device and have correct dtype
            embeddings = embeddings.to(self.device).float()
            logger.info(f"✅ Loaded embeddings from file: {embeddings.shape}")
        else:
        ...
        if os.path.exists(username_file):
            ...
            logger.info(f"✅ Loaded usernames from file: {usernames}")
        else:
    ...
    def inference(self, face_tensor, threshold=None):
        ...
        # First, try using database for direct vector search if available
        try:
            with torch.no_grad():
                detect_embeds = self.model(face_tensor.unsqueeze(0).to(self.device))
            
            # Try database similarity search first
            username, similarity = self.db.find_similar_user(detect_embeds.squeeze(0), threshold)
            
            if username is not None:
                # Find index in names list for compatibility
                try:
                    idx = self.names.index(username)
                    distance = 1 - similarity
                    confidence_percentage = similarity * 100
                    return idx, distance, confidence_percentage
                except ValueError:
                    # User found in DB but not in loaded names list, need to reload
                    logger.warning(f"User {username} found in DB but not in loaded names. Reloading data...")
                    self.embeddings, self.names = self.load_faceslist()
                    try:
                        idx = self.names.index(username)
                        return idx, 1 - similarity, similarity * 100
                    except ValueError:
                        # Still can't find, fall back to traditional method
                        pass
        except Exception as e:
            logger.debug(f"Database inference failed, falling back to tensor comparison: {e}")
        
        # Fall back to traditional tensor comparison method
    ...
    async def process_frame(self, client_id, frame, frame_count):
        ...
        # Add start time for processing time measurement
                        ...
                        if face_tensor is not None:
                            ...
                            if is_good_quality:
                                ...
                                if idx != -1:
                                    ...
                                    # New code with database logging
                                    user_name = self.names[idx]
                                    tracker_state.update_recognition_result(
                                        # name=self.names[idx],
                                        name=user_name, 
                                        confidence=confidence_percentage, 
                                    ...
                                    try:
                                        ...
                                        logger.debug(f"Failed to log recognition: {e}")
                                else:
                                    tracker_state.update_recognition_result(
                                        name="Unknown", 
                                        confidence=confidence_percentage, 
...
async def main():
    try:
        ...
        server = FaceRecognitionWebSocketServer(db_config=db_config)
